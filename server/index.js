...
app.post("/register", (req, res) => {
  ...
  let mysql = "SELECT COUNT(*) as NUMREG from games name WHERE name = ? AND cost = ? AND category = ?";
  db.query(mysql, [name, cost, category], (err, result) => {
    if (result[0].NUMREG > 0) {
      console.log("JA TA CADASTRADO");
      res.send(result)

    } else {
      mysql = "INSERT INTO games ( name, cost, category) VALUES (?, ?, ?)";
      db.query(mysql, [name, cost, category], (err, result) => {
        res.send(result);
      });
    }
